...
def generate_public_private_key():
    ...
    seed_large_number(private_seed)
    priv = generate_random_number_unsigned(bits=256)
    pub = priv * curve.g

    # Pubkey string would be pub.x, public_seed      Fixed at 768 bits
    # pub.y can then be solved for with Tonelli–Shanks algorithm
    # This is necessary because an attacker can read the passed data,
    # And check that the (pub.x, pub.y) is on the curve generated by public_seed
...
print(np.char.partition("Hello", sep=""))